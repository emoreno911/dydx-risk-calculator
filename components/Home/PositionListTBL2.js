...
const PositionItem = ({data, positions, accountCollateral, removePosition}) => {
	...
	return (
		<div className="row">
			...
			<div><span>{liquidation < 0 ? '-' : toFixedIfNecessary(liquidation, 4)}/{oraclePrice}</span></div>
			<div><span>{pnl} ({roe}%)</span></div>
			<div><span>{orderPrice}</span></div>
			<div>
